...
| `channels` | 默认 Standard User 可以管理自己的 Channel 连接。 |
...
Provider Skill 执行前应确认：用户拥有启用角色；Skill 对至少一个角色启用；Provider 实例对至少一个角色开放；Provider 实例启用；需要用户凭证时用户已配置。

## 系统管理的内置角色 {#system-managed-built-in-roles}

`admin` 和 `user` 是系统管理的内置角色。大多数模块会保持规范默认形态，Skill 和 Provider 等运行时访问模块可由管理员管理。自定义管理策略应使用自定义角色。
