...
SmartCMP Provider 将 AtlasClaw 连接到 SmartCMP 云管理平台工作流，支持资源申请、已提交申请状态查询、审批、目录查询、动态资源分析与操作、告警与资源健康分析、成本优化、资源优先的安全分析和 Security 合规违规工作流。上下文绑定的编辑辅助能力还可以读取指定 SmartCMP 定义，生成完整替换内容供人工审查和复制。
...
SmartCMP 被配置为 HostApp Provider 时，AtlasClaw 会提供共享同一套 SmartCMP Cookie 认证的独立菜单 UI 和悬浮 UI。菜单 UI 提供完整 Chat，悬浮 UI 则动态跟随受支持的 SmartCMP 页面。SmartCMP Provider 的路由定义把当前页面绑定到审批、申请、告警、费用建议、安全违规集合、资源、工单或受支持的编辑器对象，并展示符合对象当前状态的操作。详见[内嵌菜单与悬浮 UI](../embedded-menu-and-floating-ui.md)。
...
| Editor assistance | 读取受支持的表单、脚本、优化策略和蓝图组件定义，生成完整替换内容供人工复制。 |
